...
Experigen.make_into_trial = function (that) {
	...
	that.makeScale = function(obj) {
		...
		for (var i=0; i<buttons.length; i+=1) {
			...
			str += '<input type="button" value=" '+ buttons[i] +' " id="' + Experigen.screen().responses + 'button' + i + '" class="scaleButton' + direction + '" onClick="Experigen.screen().recordResponse(' + Experigen.screen().responses + "," + "'" + buttons[i] + "'" + ');Experigen.screen().continueButtonClick(this);Experigen.screen().giveFeeback(Experigen.screen().responses)">';
	...
	that.giveFeeback = function (responseNumber) {
		$("#feedback").html("");
	}
	...
	that.makeSoundButton = function (obj) {
        ...
        // (Tal Linzen:) disabling the button is not a good idea because
        // the soundmanager2 plugin sometimes doesn't work
        //str += ' onClick="Experigen.screen().playSound(\'' + soundID + '\',this); this.disabled = true;"'
